...
- **首尾帧衔接约束**（当模型模式为 `firstLastFrame` 时激活）：分镜间的「角色动作」列必须使第 N 镜的动作终点状态自然过渡到第 N+1 镜的动作起点状态。相邻分镜的出场与入场应有视觉承接——主体位置不得跳变、朝向变化必须通过「角色动作」中的转身/转头衔接、光影趋势方向一致（如上一镜"余晖渐暗"则下一镜不能突然"强光照射"，除非剧本有场景切换或时间跳切）

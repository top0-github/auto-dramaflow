...
#### 导演风格顾问（director_skills）

本系统集成了 31 位世界著名导演的风格档案，位于 `director_skills/` 目录。在制定导演规划时，**必须根据项目类型匹配 1-3 位导演风格作为创意参考**：

**类型→导演推荐映射：**

| 项目类型/题材 | 推荐导演组合 | 各导演侧重 |
|-------------|-------------|-----------|
| 悬疑/惊悚 | 大卫·芬奇 + 克里斯托弗·诺兰 + 杜琪峰 | 暗黑心理 + 结构解谜 + 静态张力 |
| 都市爱情 | 王家卫 + 侯孝贤 + 是枝裕和 | 感官诗意 + 长镜凝视 + 日常温情 |
| 武侠/古装 | 张艺谋 + 徐克 + 黑泽明 | 色彩仪式 + 奇幻视觉 + 史诗调度 |
| 科幻/奇幻 | 克里斯托弗·诺兰 + 詹姆斯·卡梅隆 + 宫崎骏 | 时空解构 + 技术奇观 + 童真想象 |
| 犯罪/黑色 | 杜琪峰 + 奉俊昊 + 昆汀·塔伦蒂诺 | 宿命冷峻 + 阶级隐喻 + 对白张力 |
| 家庭/伦理 | 是枝裕和 + 李安 + 小津安二郎 | 血缘伦理 + 文化融合 + 物哀日常 |
| 喜剧 | 周星驰 + 姜文 + 伍迪·艾伦 | 无厘头悲悯 + 黑色幽默 + 知识分子喜剧 |
| 动作/商业大片 | 詹姆斯·卡梅隆 + 吴宇森 + 徐克 | 感官冲击 + 暴力美学 + 天马行空 |
| 文艺/艺术 | 安德烈·塔可夫斯基 + 英格玛·伯格曼 + 费德里科·费里尼 | 神性静观 + 信仰叩问 + 梦境美学 |
| 史诗/历史 | 黑泽明 + 斯皮尔伯格 + 张艺谋 | 宏大调度 + 人文关怀 + 仪式美学 |
| 恐怖/心理 | 大卫·芬奇 + 朴赞郁 + 斯坦利·库布里克 | 冷峻控制 + 华丽暴力 + 精密布局 |

**使用方法**：
1. 根据项目类型从映射表中选出匹配导演，调用 `activate_skill` 工具加载对应的导演风格文件
2. 参照导演的核心视觉语言，融入规划中对应的视觉维度
3. 同项目不同段落可选不同导演侧重；但全片需保持 1 位主导视觉导演作为统一锚点

> **注意**：导演风格是创意参考而非硬性规范。当导演风格建议与「导演具象化原则」冲突时，以具象化原则为准。光影/色调方案仍由场景图原生承担，导演风格中的色彩建议作为方向性参考即可。

### 执行流程

1. 根据项目类型匹配导演映射表，调用 `activate_skill` 加载 1-3 位导演风格文件；同时加载已有风格技法参考（`director_planning_narrative`、`director_planning_style`）。获取 `script` 和 `assets`。所有规划内容以加载的风格参考为基准，冲突时以导演风格技法参考为准。

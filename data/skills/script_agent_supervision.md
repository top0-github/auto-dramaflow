...
> 本节使用 `director_skills/` 目录下的导演风格档案，从大师视角对剧本进行创意层面的审阅。**创意审阅不替代格式审核，作为独立章节输出**。

### 数据准备

1. 根据项目类型从以下映射表选出 2-3 位匹配导演
2. 调用 `activate_skill` 加载对应的导演风格文件
3. 调用 `get_planData` 获取骨架 / 改编策略 / 剧本

| 项目类型 | 推荐导演 |
|---------|---------|
| 悬疑/惊悚 | 大卫·芬奇、克里斯托弗·诺兰、杜琪峰 |
| 都市爱情 | 王家卫、侯孝贤、是枝裕和 |
| 古装/武侠 | 张艺谋、徐克、黑泽明 |
| 科幻/奇幻 | 克里斯托弗·诺兰、詹姆斯·卡梅隆、宫崎骏 |
| 犯罪/黑色 | 杜琪峰、奉俊昊、昆汀·塔伦蒂诺 |
| 家庭/伦理 | 是枝裕和、李安、小津安二郎 |
...
| 维度 | 关注点 | 提问方式 |
|------|--------|---------|
| 叙事节奏 | 故事推进速度是否适合该导演的风格节奏 | "如果用{导演}的节奏来改编，这里应加快还是放慢？" |
| 视觉可拍性 | 剧本描述是否能用镜头语言表现 | "{导演}会怎么拍这一段？现在写的能拍出来吗？" |
| 情绪密度 | 情感爆点是否足够密集和有力 | "按照{导演}的手法，这场的情绪冲击力够不够？" |
| 镜头语言 | 构图、景别、运镜是否有提升空间 | "{导演}在这场戏通常会用什么镜头语言？" |
| 跨场连续性 | 前后场次在视觉和情绪上是否连贯 | "按照{导演}的叙事逻辑，这两场之间的过渡是否自然？" |
...
| # | 维度 | 审阅意见 | 参考导演 |
|---|------|---------|---------|
| 1 | {维度} | {从导演视角提出的具体建议} | {导演名} |
```

> **重要**：本审阅仅提供创意建议，不产生严重等级标记。用户自行决定是否采纳。
...
## 十、史诗战争场景技法 (融合自 short-drama-ai-skills)

### 战前仪式

| 要素 | 方案 |
|------|------|
| 镜头 | 广角远景 → 缓推至将领面部特写 → 缓拉展现全军 |
| 色调 | 低饱和 + 灰绿偏移（战场厚重风） |
| 光影 | 侧逆光 + 强对比 + 烟雾弥漫 |
| 氛围 | 沉闷鼓声、旗幡猎猎、马匹不安的响鼻 |
...
| 构图 | 对称构图——两军左右各占画面一半 |
| 空间 | 中间留出"即将被填满的空地" |
| 景别 | 远景定场 → 中景方阵细节 → 近景将领眼神 |
| 节奏 | 极度缓慢——对峙越长，开战瞬间越有冲击力 |

### 冲锋接战

| 要素 | 方案 |
|------|------|
| 镜头运动 | 缓推（阵列前进）→ 快推（冲刺）→ 瞬间闪切（接战第一击） |
| 时长反差 | 冲锋 3-5s（蓄力）→ 接战 0.5-1s（释放） |
| 特效 | 尘土飞扬、火星飞溅、丁达尔光束穿透烟尘 |

### 战场残局

| 要素 | 方案 |
|------|------|
| 镜头 | 缓拉俯瞰——从地上的遗落兵器拉到满目疮痍的战场全景 |
| 色调 | 去饱和 + 天空血红/阴沉灰 |
| 氛围 | 薄雾 + 余烬飞散 + 乌鸦/秃鹫盘旋 |
| 音效 | 静——只剩风声和零星的火烧声 |
...
1. **尺度对比** — 每个战斗段落必须有至少 1 个"渺小个体 vs 宏大战场"的镜头
2. **烟雾即史诗** — 战场永远有烟雾：硝烟、尘土、雾气——它们让光束可见、空间有深度
3. **不拍全过程** — 史诗战争不展示每一步，只展示关键节点：对峙→冲锋→接战→残局
4. **有人才有史诗** — 远景后面必须跟一个人物近景，"从天地到眼神"
